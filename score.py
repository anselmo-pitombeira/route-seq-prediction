...
##Essa função tem implementação muito lenta 150 ms por execução
...
def erp_per_edit_helper(actual,sub,matrix,g=1000,memo=None):
    ...
    ##actual_tuple=tuple(actual)
    ##sub_tuple=tuple(sub)
    actual_tuple=actual   ##Notice we pass tuples directly
    sub_tuple=sub
    if (actual_tuple,sub_tuple) in memo:
        d,count=memo[(actual_tuple,sub_tuple)]
        return d,count
...
#     #actual_tuple=tuple(actual
#     # actual_tuple=list(actual)
#     #actual_tuple=np.array(actual)
#     #actual_tuple=(*actual,)
#     ac_len = len(actual)
#     actual_tuple = to_fixed_tuple(actual, ac_len)
#     #sub_tuple=tuple(sub)
#     # sub_tuple=list(sub)
#     #sub_tuple=(*sub,)
#     sub_len = len(sub)
#     sub_tuple = to_fixed_tuple(sub, sub_len)
#     #sub_tuple=np.array(sub)
#     if (actual_tuple,sub_tuple) in memo:
#         d,count=memo[(actual_tuple,sub_tuple)]
#         return d,count
#     if len(sub)==0:
...
# @njit    ##Parece que o njit está é deixando mais lento
